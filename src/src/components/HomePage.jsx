...
export function Competences() {
  return (
    <Section id="competences" title="Compétences" subtitle="Systèmes, réseaux, sécurité, supervision et support">
      {/* Schéma skills-sphere */}
      <div className="flex flex-col lg:flex-row gap-10 items-center mb-12">
        <div className="flex-shrink-0 flex flex-col items-center">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-xl shadow-black/30">
            <img
              src="/assets/skills-sphere.png"
              alt="Sphère de compétences technologiques"
              className="w-64 h-64 object-contain"
            />
          </div>
          <p className="text-slate-500 text-xs mt-3 text-center">Écosystème technologique</p>
        </div>
        <div className="flex-1">
          <p className="text-slate-300 leading-relaxed mb-4">
            Mon parcours en BTS SIO SISR m'a permis de développer une maîtrise solide des infrastructures systèmes et réseaux, de la virtualisation, de la supervision et de la cybersécurité.
          </p>
          <div className="flex flex-wrap gap-2">
            {["Debian 12","Linux","Zabbix","Kali Linux","SSH","VPN","VirtualBox","GLPI","OCS","Windows Server","Cisco","GPO","FileZilla FTPS","HTML/CSS","Figma"].map((t) => (
              <span key={t} className="bg-sky-500/10 text-sky-400 border border-sky-500/20 text-xs font-mono px-3 py-1 rounded-full">
                {t}
              </span>
            ))}
          </div>
