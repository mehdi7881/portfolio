  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 space-y-16">
        ...
        {/* Infos RP + lien production */}
        {(projet.numero || projet.lienProduction) && (
          <div className="bg-slate-900 border border-sky-500/20 rounded-2xl p-6">
            <h2 className="text-white font-black text-xl mb-4 flex items-center gap-2">
              <span className="text-sky-400">📋</span> Fiche de realisation professionnelle
            </h2>
            <div className="grid sm:grid-cols-3 gap-4 mb-4">
              {projet.numero && (
                <div><p className="text-slate-500 text-xs uppercase tracking-wider">Numero</p><p className="text-white font-semibold">{projet.numero}</p></div>
              )}
              {projet.periode && (
                <div><p className="text-slate-500 text-xs uppercase tracking-wider">Periode</p><p className="text-white font-semibold">{projet.periode}</p></div>
              )}
              {projet.lieu && (
                <div><p className="text-slate-500 text-xs uppercase tracking-wider">Lieu</p><p className="text-white font-semibold">{projet.lieu}</p></div>
              )}
            </div>
            {projet.lienProduction && (
              <a href={projet.lienProduction} target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-2 bg-sky-500/10 text-sky-400 border border-sky-500/25 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-sky-500/20 transition-colors">
                Acceder aux productions →
              </a>
            )}
          </div>
        )}
        ...
        {projet.ficheWord && (
          <div>
            <h2 className="text-white font-black text-xl mb-4 flex items-center gap-2">
              <span className="text-sky-400">📄</span> Apercu du document officiel
            </h2>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="bg-slate-800 px-5 py-3 flex items-center justify-between border-b border-slate-700">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center text-blue-400 text-sm">W</div>
                  <div>
                    <p className="text-white text-sm font-semibold">Fiche descriptive BTS SIO - Epreuve E6</p>
                    <p className="text-slate-400 text-xs">Session 2026 - AHMIDDOUCH Mehdi</p>
                  </div>
                </div>
                <a href={projet.ficheWord} download
                  className="flex items-center gap-1.5 bg-sky-500 hover:bg-sky-400 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-colors">
                  Telecharger
                </a>
              </div>
